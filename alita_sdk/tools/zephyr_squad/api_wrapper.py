...
class ZephyrSquadApiWrapper(BaseToolApiWrapper):
    ...
    def toolkit_config_schema(self) -> BaseModel:
        available_tools_dict = {x['name']: x['args_schema'].model_json_schema() for x in self.get_available_tools()}
        #
        return create_model(
            "zephyr_squad",
            account_id=(SecretStr, Field(description="AccountID for the user that is going to be authenticating")),
            access_key=(SecretStr, Field(description="Generated access key")),
            secret_key=(SecretStr, Field(description="Generated secret key")),
            # TODO deduplicate selected tools for toolkits
            selected_tools=(List[Literal[tuple(available_tools_dict)]], Field(default=[], json_schema_extra={'args_schemas': available_tools_dict})),
    ...
    def get_available_tools(self):
        return [
            {
                "name": "get_test_step",
                "description": "Retrieve details for a specific test step in a Jira test case.",
                "args_schema": ProjectIssueStep,
                "ref": self.get_test_step,
            },
            {
                "name": "update_test_step",
                "description": "Update the content or a specific test step in a Jira test case.",
                "args_schema": UpdateTestStep,
                "ref": self.update_test_step,
            },
            {
                "name": "delete_test_step",
                "description": "Remove a specific test step from a Jira test case.",
                "args_schema": ProjectIssueStep,
                "ref": self.delete_test_step,
            },
            {
                "name": "create_new_test_step",
                "description": "Add a new test step to a Jira test case.",
                "args_schema": CreateNewTestStep,
                "ref": self.create_new_test_step,
            },
            {
                "name": "get_all_test_steps",
                "description": "List all test steps associated with a Jira test case.",
                "args_schema": ProjectIssue,
                "ref": self.get_all_test_steps,
            },
            {
                "name": "get_all_test_step_statuses",
                "description": "Retrieve all possible statuses for test steps in Jira.",
