"""
Skill Router Tool for LangChain integration.

This module provides a single LangChain tool that exposes the entire
skills registry system to agents, allowing them to list, describe,
and execute skills with proper input handling and result formatting.
"""
...
class SkillRouterInput(BaseModel):
    """Input schema for the Skill Router tool."""

    mode: Literal["list", "describe", "execute"] = Field(
        description="Operation to perform: 'list' shows all skills, 'describe' shows skill details, 'execute' runs a skill"
    )
    ...
    task: Optional[str] = Field(
        default=None,
        description="Task or question for the skill (required for execute mode)"
    )
    ...
    # Filtering options (for list mode)
...
class SkillRouterTool(BaseTool):
    """
    LangChain tool for skill registry operations.

    This tool provides a unified interface for agents to interact with
    the skills system, including listing, describing, and executing skills.
    """
...
Examples:
- {"mode": "list"} - Show all available skills
- {"mode": "list", "skill_type": "agent", "capability": "jira"} - Show agent skills with Jira capability
- {"mode": "describe", "skill_name": "jira_triage"} - Get details about jira_triage skill
- {"mode": "execute", "skill_name": "jira_triage", "task": "Analyze issue PROJ-123", "context": {"issue_key": "PROJ-123"}}
- {"mode": "execute", "skill_name": "my_skill", "task": "Process data", "execution_mode": "remote"} - Override execution mode
"""

    args_schema: Type[BaseModel] = SkillRouterInput
    return_direct: bool = True

    # Declare Pydantic fields for the tool components
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Initialize the Skill Router tool.
...
Respond with only the skill name that best matches the task. If no skill is appropriate, respond with "no_match"."""
    ...
    def _run(
        ...
        **kwargs
        ...
        """
        Execute the skill router operation.
        ...
        try:
            if mode == "list":
                return self._handle_list(skill_type, capability, tag)
            elif mode == "describe":
                return self._handle_describe(skill_name)
            elif mode == "execute":
                return self._handle_execute(
                    skill_name, task, context, chat_history,
                    state_variables, execution_mode, enable_callbacks
                )
            else:
                return f"Error: Invalid mode '{mode}'. Use 'list', 'describe', or 'execute'."

        except Exception as e:
            logger.error(f"SkillRouterTool error: {e}")
    ...
    def _handle_describe(self, skill_name: Optional[str]) -> str:
        """Handle describe mode operation."""
        if not skill_name:
            return "Error: skill_name is required for describe mode."
