...
| **Primary Input(s)** | `{{PAGE_IDS_TO_UPDATE}}, {{NEW_CONTENTS_TO_INSERT}}` | Inputs derived from args_schema |
...
- Target pages with `{{PAGE_IDS_TO_UPDATE}}` exist and are accessible
- `{{NEW_CONTENTS_TO_INSERT}}` length equals `{{PAGE_IDS_TO_UPDATE}}` length

## Test Steps & Expectations

### Step 1: Execute the Tool

Execute `update_pages` with `page_ids={{PAGE_IDS_TO_UPDATE}}`, `new_contents={{NEW_CONTENTS_TO_INSERT}}`.
...
Execute the `read_page_by_id` tool for each page from `{{PAGE_IDS_TO_UPDATE}}`.
Verify that the output contains the updated page content as text.

### Step 4: Delete created pages

Execute `delete_page` for each page from `{{PAGE_IDS_TO_UPDATE}}`.
