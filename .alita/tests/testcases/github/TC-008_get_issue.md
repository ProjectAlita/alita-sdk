...
Execute the `get_issue` tool with issue number `13` against the target repository.
...
**Expectation:** The output is an object containing the following fields: `number`, `title`, `body`, `state`, `url`, `created_at`, `updated_at`, `comments`, `labels`, and `assignees`. The `number` field should match the requested `13`.
